...
namespace RSQL
{
	...
	{		
		for (unsigned int i = 0; i < v.size(); ++i)
		{
			...
			while (it != v[i].end())
			{
				...
				{
					case TYPE_LONG: {stringstream s; it->second->getValue(l); s << l; l = s.str().size(); tmp.push_back(s.str()); break;}
					case TYPE_TEXT: {string s; it->second->getValue(s); l = s.size(); tmp.push_back(s); break;}
					default: l = 0; 
				}
	...
	string out(_RTable t)
	{
		...
		unsigned int totalW; for (unsigned int i = 0; i < sizes.size(); ++i) {totalW += (3 + sizes[i]);} totalW += 2;
		for (unsigned int i = 0; i < totalW-1; ++i) ss << "*"; ss << endl;
...
// 			{
			...
			catch (RException* ex) {send(*sock, ex->message().c_str(), strlen(ex->message().c_str()), 0);}
			bzero(buf, 32768);
			//}
			//break;
		}
